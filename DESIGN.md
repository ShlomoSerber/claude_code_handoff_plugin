...
Per-model weekly limits: the status JSON builder in 2.1.280 fills only `five_hour`, `seven_day` (and `spend_limit` on a gateway). The per-model bucket ("Fable") exists only in `GET https://api.anthropic.com/api/oauth/usage` (header `anthropic-beta: oauth-2025-04-20`, Claude Code's OAuth token from `~/.claude/.credentials.json`), as `limits[]` entries with `kind: weekly_scoped`, `scope.model.display_name` and `percent`. This is the call `/usage` makes. The status line runs on every UI update and must stay fast, so it never calls the network itself: when the cache in `~/.cache/claude-handoff/usage.json` is older than 5 minutes, it starts a detached fetch (one at a time, lock file) and prints from the cache it has. An expired token or a failed fetch keeps the old cache; the script never refreshes tokens. The endpoint is undocumented and may change.
