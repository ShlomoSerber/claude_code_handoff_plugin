...
- In bypass-permissions mode, peer messages are held for review unless the sender is a descendant of the session (`selfSent`). A background process detached from the hook gets reparented and is held. On Linux, `selfSent` needs the sender's process ancestry, which Claude Code reads after the message line arrives. A sender that exits right after writing is often gone by then, and the message is held (seen in about half the runs). The hook keeps the connection open for 1.5 s after writing; with that, a synchronous send on `source: clear` was delivered 3 of 3 times. The same send on `source: startup` is held ("from an unidentified session"), so the hook uses the socket only on `clear`.
...
Per-model weekly limits: the status JSON builder in 2.1.280 fills only `five_hour`, `seven_day` (and `spend_limit` on a gateway). The per-model bucket ("Fable") exists only in `GET https://api.anthropic.com/api/oauth/usage` (header `anthropic-beta: oauth-2025-04-20`, Claude Code's OAuth token from `~/.claude/.credentials.json`), as `limits[]` entries with `kind: weekly_scoped`, `scope.model.display_name` and `percent`. This is the call `/usage` makes. The same response carries `kind: session` and `kind: weekly_all`; the cache stores them too, because the status JSON has no `rate_limits` until the session's first reply. A fresh session would otherwise show only the per-model limit. The status line runs on every UI update and must stay fast, so it never calls the network itself: when the cache in `~/.cache/claude-handoff/usage.json` is older than 5 minutes, it starts a detached fetch (one at a time, lock file) and prints from the cache it has. An expired token or a failed fetch keeps the old cache; the script never refreshes tokens. The endpoint is undocumented and may change.

The built-in yellow warning ("Context low (N% remaining)") only appears near auto-compact, which on a 1M model is ~967k; hence the custom line.
