...
| Thing | Where / value |
|---|---|
| Purpose | `/handoff-clear <next prompt>` writes a compressed AI-only state note and parks the prompt; user types `/clear`; the hook puts the prompt in the fresh session's context and the user's next message (`dale`) starts it. Goal: keep sessions small. |
...
| No auto-/clear | Nothing can run `/clear` for the user (docs, binary, socket `skipSlashCommands`, Wayland; DESIGN.md §6). Do not try again. The skill must never print `/clear` alone as if it ran. The hook prints the parked prompt as context; it does NOT use `$CLAUDE_CODE_MESSAGING_SOCKET` (removed 0.5.0: Claude Code labels every socket message as another session's, not the user's; user wants the prompt to be fully theirs, DESIGN.md §6). Parked prompt < 2 h old, never consumed on `resume`. |
