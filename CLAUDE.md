...
| No auto-/clear | Nothing can run `/clear` for the user (docs, binary, socket `skipSlashCommands`, Wayland; DESIGN.md §6). Do not try again. The skill must never print `/clear` alone as if it ran. After `/clear` the hook sends the parked prompt as a user message through `$CLAUDE_CODE_MESSAGING_SOCKET`, synchronously, only on `source: clear` (a detached sender, or any sender on `startup`, is held in bypass mode). On `startup` it prints the old context line instead. Parked prompt < 2 h old, never consumed on `resume`. |
